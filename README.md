...
- 支持微信文字、普通链接、公众号链接、小红书分享文本和小宇宙公开单集。
...
- 读取公开、免费的单集标题、播客、主播、介绍和时长。音频转写目前默认暂停，只保存链接和节目资料；选定更快的转写方案后再由服务端显式开启。
- 付费或私密单集不会下载；音频地址只允许小宇宙的 `xyzcdn.net` HTTPS 域名。
- CDN 音频地址不进入笔记或任务数据；原音频只进入 Worker 私有临时目录，切成 28 秒音频片段后立即删除。
- 转写片段在成功、失败或取消后立即到期，最长兜底保留四小时。
- 长期只保留小宇宙单集页链接、节目元信息和转写文字。
