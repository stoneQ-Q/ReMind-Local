...
- 读取公开、免费的单集标题、播客、主播、介绍和时长。音频转写默认暂停；服务端显式开启后使用百炼 `paraformer-v2` 异步转写。
- 付费或私密单集不会下载；音频地址只允许小宇宙的 `xyzcdn.net` HTTPS 域名。
- CDN 音频地址只在当前解析任务内提交给百炼，不进入笔记、数据库或任务数据；ReMind 不下载或保存原始音频。
- 长期只保留小宇宙单集页链接、节目元信息、完整逐字稿和时间戳片段。
