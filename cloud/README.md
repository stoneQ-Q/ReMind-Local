...
当前 Worker 只执行注册过的任务类型，第一种仍是零费用的 `system.noop`。通用执行器提供：
...
API 提供 `GET /api/v1/jobs/:jobId` 查询当前用户自己的任务状态，以及 `POST /api/v1/jobs/:jobId/cancel` 请求取消。当前尚未注册微信、网页、图片、视频或语音处理器，因此通用执行器不会访问这些真实数据，也不会产生第三方费用。
