...
当前 Worker 只执行注册过的任务类型，包括零费用的 `system.noop` 和按连接调度的 `wechat.poll`。通用执行器提供：
...
API 提供 `GET /api/v1/jobs/:jobId` 查询当前用户自己的任务状态，以及 `POST /api/v1/jobs/:jobId/cancel` 请求取消。网页、图片、视频和语音处理器尚未注册。
