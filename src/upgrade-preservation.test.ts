...
describe('ReMind in-place upgrade preservation', () => {
  it('pins the application, database, WeChat, cloud, and Obsidian identities', async () => {
    ...
    expect(
      wechatDeviceIdStorageKey('http://192.168.50.23:8787'),
    ).toBe('remind.wechat.device-id.783d4d72');
    expect(
      wechatDeviceSecretStorageKey('http://192.168.50.23:8787'),
    ).toBe('remind.wechat.device-secret.783d4d72');
    expect(
      cloudSessionStorageKey(
  ...
  it('reads existing SecureStore values after the code upgrade', async () => {
    process.env.EXPO_PUBLIC_REMIND_LOCAL_API_URL =
      'http://192.168.50.23:8787';
    process.env.EXPO_PUBLIC_REMIND_CLOUD_API_URL =
      'https://cloud.remind.example';

    secureValues.set(
      'remind.wechat.device-id.783d4d72',
      'existing-device-id',
    );
    secureValues.set(
      'remind.wechat.device-secret.783d4d72',
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      'http://192.168.50.23:8787/api/devices/existing-device-id/status',
